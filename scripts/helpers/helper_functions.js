class HelperFunctions {
	static getPosX(width_percent) {
		var window_width = $(window).width();
		
		return window_width * (width_percent/100);
	}
	
	static getPosY(height_percent) {
	...
	static getRandomNumber(min_num, max_num) {
		return Math.floor(Math.random() * (max_num - min_num + 1)) + min_num;
	}
	
	static getNormalizedXYDirection(posXYStart, posXYEnd) {
	...
	static getXYDifference(posXYStart, posXYEnd) {
		return [posXYEnd[0]-posXYStart[0], posXYEnd[1], posXYStart[1]];
	}
	
	static floatCompare(compare_type, val, compare_to, float_error) {
